...
const Question = ({ gameState, socket, setInfoBar }) => {
  ...
  useEffect(() => {
    if (profile?.id in Object.keys(gameState.roomInfo.questions[gameState.roomInfo.round].answers)){
      setIsSubmitted(true)
    }
  }, [gameState, profile]);
  ...
  return (
    <div>
      ...
      <textarea
        ...
        hidden={isSubmitted}
      /><br/>
      <button onClick={handleSendAnswer} disabled={isSubmitted}>Send Answer</button>
      {isSubmitted ? <>
